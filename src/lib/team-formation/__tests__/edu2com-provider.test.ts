...
describe("edu2com team formation provider", () => {
  beforeEach(() => {
    ...
    markProcessingMock.mockResolvedValue({
      ...
      provider: "edu2com",
      status: "PROCESSING",
      replyPostUrl: "https://example.com/api/edu2com/webhook?requestId=req-123&token=test",
    });
  ...
  afterEach(() => {
    ...
    if (originalBaseUrl === undefined) delete process.env.EDU2COM_WEBHOOK_BASE_URL;
    else process.env.EDU2COM_WEBHOOK_BASE_URL = originalBaseUrl;
    if (originalSecret === undefined) delete process.env.EDU2COM_WEBHOOK_SECRET;
  ...
  it("sends a signed webhook URL and marks the request as processing", async () => {
    ...
    expect(callBackgroundMock).toHaveBeenCalledWith(
      expect.objectContaining({
        replyPostUrl: expect.stringContaining("/api/edu2com/webhook?requestId=req-123&token="),
      }),
      expect.any(Object),
    );
