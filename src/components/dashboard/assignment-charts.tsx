...
const GenderPieChart = dynamic(
  ...
  {
    loading: () => <div className="flex-1 animate-pulse bg-gray-100 rounded-lg h-[200px]" />,
  },
);
...
const TopicPreferencesPieChart = dynamic(
  () =>
    import("@/components/dashboard/chart/topic-preferences-pie-chart").then((mod) => ({
      default: mod.TopicPreferencesPieChart,
    })),
  {
    loading: () => <div className="flex-1 animate-pulse bg-gray-100 rounded-lg h-[200px]" />,
  },
...
export function AssignmentCharts({ stats, isStudent }: AssignmentChartsProps) {
  ...
  return (
    <Suspense fallback={<div className="flex-1 animate-pulse bg-gray-100 rounded-xl" />}>
      ...
      <div className="flex gap-x-4 flex-1 min-h-0">
        <div className="flex flex-col border shadow-sm rounded-xl p-4 flex-1 min-h-0">
          <div className="shrink-0 mb-3">
            <span className="text-neutral-500 font-light text-base block">{t("averageLabel")}</span>
            <h1 className="text-neutral-800 font-medium text-xl">{t("skillsTitle")}</h1>
          </div>
          <SkillsBarChart skills={stats.skills} skillsReady={stats.skillsReady} />
        </div>

        <div className="flex flex-col border shadow-sm rounded-xl p-4 flex-1 min-h-0">
          <div className="shrink-0 mb-3">
            <span className="text-neutral-500 font-light text-base block">{t("averageLabel")}</span>
            <h1 className="text-neutral-800 font-medium text-xl">{t("preferencesTitle")}</h1>
          </div>
          <TopicPreferencesPieChart topicPreferences={stats.topicPreferences} ready={isReady} />
        </div>

        <div className="flex flex-col border shadow-sm rounded-xl p-4 flex-1 min-h-0">
          <div className="shrink-0 mb-3">
            <span className="text-neutral-500 font-light text-base block">{t("averageLabel")}</span>
            <h1 className="text-neutral-800 font-medium text-xl">{t("genderTitle")}</h1>
          </div>
