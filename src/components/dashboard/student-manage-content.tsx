...
export async function getStudentManageItems(user: ExtendedUser): Promise<GroupListItem[]> {
  ...
  const enrollmentByCourseId = new Map<string, Enrollment>(
    enrollments.map((enrollment: Enrollment) => [enrollment.courseId, enrollment]),
  );

  const assignmentIds = assignments.map((assignment: Assignment) => assignment.id);
  ...
  const ownerIdsWithMissingTeamFormation = [
    ...new Set(
      assignmentsMissingTeamFormation.map((assignment: Assignment) => assignment.createdById),
    ),
  ];
  ...
  const submissionSet = new Set(
    submissions.map((submission: (typeof submissions)[number]) => submission.assignmentId),
  );

  const legacyTeamFormationsByOwner = new Map<string, typeof legacyTeamFormations>();
  ...
  const resolvedTeamFormationByAssignment = new Map<string, ResolvedTeamFormation>();
  const topicIdsByAssignment = new Map<string, string[]>();

  for (const assignment of assignments) {
    let teamFormation: ResolvedTeamFormation = assignment.teamFormationRequests[0] ?? null;

    if (!teamFormation) {
      const legacyCandidates = legacyTeamFormationsByOwner.get(assignment.createdById) ?? [];
      teamFormation =
        legacyCandidates.find((candidate: (typeof legacyTeamFormations)[number]) =>
    ...
    const response = teamFormation?.responseData as { teams?: Array<{ taskId: string }> } | null;
  ...
  const uniqueTopicIds = [
    ...new Set([...topicIdsByAssignment.values()].flatMap((topicIds: string[]) => topicIds)),
  ];
  ...
  return assignments.map((assignment: Assignment) => {
    ...
    const teamFormation = resolvedTeamFormationByAssignment.get(assignment.id) ?? null;
    ...
    const studentTeam = teamFormation?.teams.find((team: Team) =>
      team.members.some((member: Member) => member.userId === user.id),
    );
    const isInTeam = Boolean(studentTeam);
    ...
    if (studentTeam) {
      teamMembers = studentTeam.members.map((member: Member) => ({
        id: member.id,
        user: {
          id: member.user.id,
          name: member.user.name,
          mbtiType: member.user.personalityProfile?.mbtiType as string | null,
        },
      }));
      ...
      const teamIndex = teamFormation?.teams.findIndex((team: Team) => team.id === studentTeam.id);
      const topics = topicsByAssignment.get(assignment.id);
      if (teamIndex !== undefined && teamIndex >= 0 && topics && topics[teamIndex]) {
...
export async function StudentManageContent({ user }: StudentManageContentProps) {
  const t = await getTranslations("dashboard.studentManage");
  const items = await getStudentManageItems(user);
