...
describe("api-utils", () => {
  ...
  describe("withRole", () => {
    ...
    it("denies access when user lacks required role", async () => {
      ...
      const { withRole } = await import("@/lib/api-utils");
      const handler = withRole("TEACHER", async () => NextResponse.json({ ok: true }));
      const res = await handler(new Request("http://localhost/x") as any, undefined as any);
  ...
  describe("withValidation", () => {
    ...
    it("returns 400 for invalid data", async () => {
      ...
      const route = withAuth(withValidation(schema, async () => NextResponse.json({ ok: true })));
  ...
  describe("requireRole", () => {
    ...
    it("throws when user lacks required role", async () => {
      ...
      expect(() => requireRole("TEACHER", user as any)).toThrow(AuthorizationError);
      expect(() => requireRole("TEACHER", user as any)).toThrow("Insufficient permissions");
    });
