...
const createCourseCatalogEntryMock = mock(async (input: { code: string; name: string }) => ({
  id: "custom-entry",
  code: input.code,
  name: input.name,
  createdAt: new Date(),
...
const prismaUserFindUniqueMock = mock(async ({ where }: any) => usersById[where.id] ?? null);
