...
export function GroupCard({ item }: GroupCardProps) {
  ...
  const detailHref = item.href ?? `/dashboard/class/${item.courseId}/assignments/${item.id}`;
  const quizHref = item.quizHref ?? `/dashboard/class/${item.courseId}/assignments/${item.id}/quiz`;

  const handleClick = () => {
    router.push(item.status === "not-started" ? quizHref : detailHref);
  };
  ...
  if (item.status === "my-group" && item.teamMembers && item.teamMembers.length > 0) {
    return (
        ...
        >
          <div className="flex flex-col gap-3">
            ...
            <div className="border rounded-2xl p-3 hover:bg-gray-50 bg-white">
              <h3 className="font-semibold text-lg text-gray-800 line-clamp-2">{item.taskTitle}</h3>
              <p className="text-sm text-muted-foreground mt-1 line-clamp-1">{item.className}</p>
            </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className={`rounded-full ${badge.color} border px-2 py-0.5 text-xs`}>{badge.text}</div>
      </div>
      <h3 className="mt-2 text-lg font-normal text-stone-900">{item.taskTitle}</h3>
