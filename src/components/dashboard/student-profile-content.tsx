...
export function StudentProfileContent({
  ...
  return (
    <div className={`bg-white ${isModal ? "rounded-3xl" : "rounded-3xl"} flex flex-col p-4 gap-4`}>
      {isModal && onClose && (
      ...
      <ProfileHeader user={student} hideImage={isModal} hideEditButton={isModal} />
      ...
      <div className="flex justify-between items-center gap-4 mb-0 pb-0">
        ...
        >
          <Smile strokeWidth={3} />
          <span className="text-md text-stone-900 font-semibold">{t("viewMbtiDistribution")}</span>
        </Button>

        {canManage && onRemoveStudent && (
          <Button onClick={onRemoveStudent} variant="destructive" className="rounded-full h-[3rem]">
