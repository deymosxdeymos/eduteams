...
describe('POST /api/debug/clear-cache', () => {
  it('requires admin and clears tag', async () => {
    mock.module('@/lib/auth', () => ({ auth: { api: { getSession: async () => ({ user: { id: 'a1' } }) } } }));
    ...
    mock.module('@/lib/prisma', () => ({ default: { user: { findUnique: async () => ({ id: 'a1', role: 'admin', isOnboarded: true }) } } }));
    const { POST } = await import('../route');
    const req = new Request('http://localhost/api/debug/clear-cache', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ tag: 'x' }) });
