...
import { getStudentCompetencyPrefills, normalizeTopicKey } from "../student-competency-profiles";
...
describe("normalizeTopicKey", () => {
  ...
  it("handles mixed case with multiple words", () => {
    expect(normalizeTopicKey("React Native Development")).toBe("react native development");
  });
});
...
describe("getStudentCompetencyPrefills", () => {
  ...
  describe("empty inputs", () => {
    it("returns empty arrays when no skills or topics provided", async () => {
      ...
      expect(mockPrisma.studentCompetencyProfile.findMany).not.toHaveBeenCalled();
    });

    it("handles only skill names", async () => {
      mockPrisma.skill.findMany.mockResolvedValueOnce([{ id: "skill-1", name: "JavaScript" }]);
