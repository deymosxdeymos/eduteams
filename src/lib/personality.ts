...
export function isValidPersonalityScores(obj: unknown): obj is PersonalityScores {
  if (typeof obj !== "object" || obj === null) return false;
...
export function calculatePersonalityScores(
  ...
  questions: Array<
    Pick<PersonalityQuestionRecord, "id" | "dimension" | "reversed" | "isAttentionCheck">
  >,
): PersonalityScores {
...
export function getMBTIType(scores: PersonalityScores): string {
  ...
  return values.map((value, index) => MBTI_LETTERS[index][value < 0 ? 0 : 1]).join("");
}
...
export function calculatePersonalityScoresFromQuestions(
  ...
  const totals: Record<"ei" | "sn" | "tf" | "pj", { sum: number; count: number }> = {
    ei: { sum: 0, count: 0 },
    sn: { sum: 0, count: 0 },
    tf: { sum: 0, count: 0 },
    pj: { sum: 0, count: 0 },
