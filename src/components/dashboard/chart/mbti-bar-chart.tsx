...
const MBTI_GROUP: Record<MbtiType, "analyst" | "diplomat" | "sentinel" | "explorer"> = {
  INTJ: "analyst",
  INTP: "analyst",
  ENTJ: "analyst",
  ENTP: "analyst",
...
function MbtiBarShape(
  ...
  const { x = 0, y = 0, width = 0, height = 0, payload, gradientIds, faceShadowId } = props;
  ...
  return (
    <>
      <div className="mb-2 flex items-center justify-between gap-4">
        <div>
          <span className="text-neutral-500 font-light text-base block">{distributionLabel}</span>
          <h1 className="text-neutral-800 font-medium text-xl">{personalityTitle}</h1>
        </div>
        <Select value={sortOrder} onValueChange={(value) => setSortOrder(value as SortOrder)}>
          <SelectTrigger size="sm" className="min-w-[180px] rounded-full bg-accent/30 gap-2 !h-11">
      ...
      <ChartContainer config={chartConfig} className="h-[180px] w-full mb-3">
        ...
        >
          <defs>
            <filter id={faceShadowId} x="-50%" y="-50%" width="200%" height="200%">
