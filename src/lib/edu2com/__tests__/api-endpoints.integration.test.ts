...
import { EDU2COM_BASE_SKILLS as BASE_SKILLS, createBasePayload } from "@/lib/edu2com/fixtures";
  ...
  () => {
    ...
    describe("POST /v1/backgroundTeamFormation - Asynchronous Endpoint", () => {
      it("returns 202 for valid background request with webhook URL", async () => {
        ...
        const res = await fetch(`${EDU2COM_BASE_URL}/v1/backgroundTeamFormation`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
