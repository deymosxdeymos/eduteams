...
export function getLocalizedApiMessage(locale: RequestLocale, key: string): string {
  const messages = locale === "en" ? enMessages : idMessages;
  const keys = key.split(".");
  let value: Record<string, unknown> | string = messages;

  for (const segment of keys) {
    value = (value as Record<string, unknown>)?.[segment] as Record<string, unknown> | string;
  }
