...
export function AssignmentActions({
  ...
  const [method, setMethod] = useState<"JUMLAH_KELOMPOK" | "JUMLAH_MHS_PER_KELOMPOK" | "">("");
  ...
  const canSubmit = Boolean(method && value && Number(value) > 0 && !shouldDisableForm);
    ...
    <div className="flex items-center gap-4 shrink-0">
      ...
      {isEditMode && canManage && (
        <div className="flex items-center gap-2">
          ...
          >
            <ArrowLeft strokeWidth={2} className="w-6 h-6 text-gray-600" />
            <span className="text-gray-900 font-semibold text-sm">{tTeams("cancelEdit")}</span>
          </Button>
          <Button
            variant="onboarding"
            className="rounded-full p-6"
          ...
          >
            {isSaving ? (
              <>
                ...
                <span className="font-semibold text-sm">{t("submitCreating")}</span>
              </>
            ) : (
              <span className="font-semibold text-sm">{tTeams("saveChanges")}</span>
            )}
          </Button>
        </div>
      )}
              ...
              <DialogContent className="border max-w-md md:max-w-xl rounded-3xl p-0 gap-0">
                ...
                <div className="p-6 pt-0 space-y-5">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-900">{t("methodLabel")}</label>
                    <Select
                      value={method}
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="JUMLAH_KELOMPOK">{t("methodByGroupCount")}</SelectItem>
                            ...
                            const groups =
                              method === "JUMLAH_KELOMPOK"
                                ...
                                : val > 0
                                  ? Math.max(
                                      1,
                                      Math.ceil((enrollmentCount ?? 0) / Math.max(1, val)),
                                    )
                                  : 0;
                            if (groups && (topicCount ?? 0) > 0 && groups !== topicCount) {
