  ...
  course: {
    findFirst: mock(async (args: any) =>
      args?.where?.id === 'c1' && args?.where?.dosenId === 'u1'
        ? { id: 'c1', shareToken: null, namaMataKuliah: 'Algoritma', kelas: 'RA' }
        : null
    ),
    update: mock(async (args: any) => ({ id: args.where.id, shareToken: args.data.shareToken })),
  },
};
...
describe('courses/[id]/share-token API', () => {
  it('GET generates token if missing and returns shareUrl', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/courses/c1/share-token') as any, { params: Promise.resolve({ id: 'c1' }) } as any);
  ...
  it('POST regenerates token', async () => {
    ...
    const req = new Request('http://localhost/api/courses/c1/share-token', { method: 'POST', headers: { 'x-forwarded-host': 'localhost' } });
