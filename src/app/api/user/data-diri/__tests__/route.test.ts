...
const prismaMock: any = {
  user: {
    findUnique: mock(async () => ({ id: 'u1', name: 'User', nimNpm: '123', role: 'mahasiswa', gender: 'MALE' })),
    update: mock(async () => ({})),
  },
};
...
describe('user/data-diri API', () => {
  it('GET returns mapped user fields', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/user/data-diri') as any, undefined as any);
  ...
  it('POST updates user and requires role-specific fields', async () => {
    ...
    const reqOk = new Request('http://localhost/api/user/data-diri', {
      ...
      body: JSON.stringify({ namaLengkap: 'U', jenisKelamin: 'laki-laki', role: 'dosen', npm: 'X' }),
    });
    ...
    const reqFail = new Request('http://localhost/api/user/data-diri', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ namaLengkap: 'U', jenisKelamin: 'perempuan', role: 'mahasiswa' }),
    });
