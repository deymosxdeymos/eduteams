...
export function generateTeamFormationCSV(data: AssignmentExportData): string {
  ...
  rows.push("");
  rows.push(`${escapeCsvValue("Assignment:")},${escapeCsvValue(data.assignmentTitle)}`);
  rows.push(
    `${escapeCsvValue("Course:")},${escapeCsvValue(`${data.courseName} - ${data.courseClass}`)}`,
  );
  rows.push(`${escapeCsvValue("Instructor:")},${escapeCsvValue(data.dosenName)}`);
  rows.push(
    `${escapeCsvValue("Formation Date:")},${escapeCsvValue(formatDate(data.formationDate))}`,
  );
  rows.push("");
  rows.push(`${escapeCsvValue("Total Students:")},${escapeCsvValue(data.metadata.totalStudents)}`);
  rows.push(`${escapeCsvValue("Total Teams:")},${escapeCsvValue(data.metadata.totalTeams)}`);
  rows.push(
  ...
  for (const team of data.teams) {
    team.members.forEach((member, memberIndex) => {
      const qualityPercent = team.quality !== null ? (team.quality * 100).toFixed(1) : "";
