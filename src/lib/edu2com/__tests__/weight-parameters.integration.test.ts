...
describeIntegration('Weight Parameters - Exhaustive Testing', () => {
  ...
  describe('Reproducibility Tests', () => {
    it('same inputs with initRandom=false produce consistent results', async () => {
      ...
      const normalizedByTask1 = normalizeTeamsForComparison(response1);
      const normalizedByTask2 = normalizeTeamsForComparison(response2);
      expect(normalizedByTask1.length).toBe(normalizedByTask2.length);

      // Membership compositions should match even if tasks swap order
      const normalizedByMembers1 = normalizeTeamsByMembers(response1);
      const normalizedByMembers2 = normalizeTeamsByMembers(response2);
