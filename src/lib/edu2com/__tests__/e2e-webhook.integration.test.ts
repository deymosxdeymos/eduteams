...
describe.skipIf(!(process.env.EDU2COM_INTEGRATION === "1" && process.env.EDU2COM_E2E === "1"))(
...
function _computeWebhookUrl(requestId: string): string {
  const baseUrl = process.env.EDU2COM_WEBHOOK_BASE_URL || "http://localhost:3000";
  const secret = process.env.EDU2COM_WEBHOOK_SECRET || "";

  // Compute HMAC token
  const token = createHmac("sha256", secret).update(requestId).digest("hex");
