...
export function ShareClassModal({ isOpen, onClose, courseData }: ShareClassModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="rounded-3xl sm:max-w-xl" showCloseButton={false}>
        <DialogHeader className="relative">
          <DialogTitle className="text-xl font-semibold">{t("title")}</DialogTitle>
          <DialogDescription className="text-sm text-black">{t("description")}</DialogDescription>
          <Button
            variant="ghost"
            size="icon"
            onClick={onClose}
                ...
                <div className="relative w-4 h-4">
                  <Copy
                    className={`absolute inset-0 transition-[opacity,transform,filter] duration-200 ${
                      copiedUrl ? "scale-50 opacity-0 blur-sm" : "scale-100 opacity-100 blur-0"
                    }`}
                    style={{
                      transitionTimingFunction: "cubic-bezier(0.25, 0.46, 0.45, 0.94)",
                    }}
                  ...
                  <Check
                    className={`absolute inset-0 transition-[opacity,transform,filter] duration-200 ${
                      copiedUrl ? "scale-100 opacity-100 blur-0" : "scale-50 opacity-0 blur-sm"
                    }`}
                    style={{
                      transitionTimingFunction: "cubic-bezier(0.25, 0.46, 0.45, 0.94)",
                    }}
            ...
            <div className="flex gap-2 text-gray-400">
              ...
              >
                <div className="relative w-4 h-4">
                  <Copy
                    className={`absolute inset-0 transition-[opacity,transform,filter] duration-200 ${
                      copiedToken ? "scale-50 opacity-0 blur-sm" : "scale-100 opacity-100 blur-0"
                    }`}
                    style={{
                      transitionTimingFunction: "cubic-bezier(0.25, 0.46, 0.45, 0.94)",
                    }}
                  ...
                  <Check
                    className={`absolute inset-0 transition-[opacity,transform,filter] duration-200 ${
                      copiedToken ? "scale-100 opacity-100 blur-0" : "scale-50 opacity-0 blur-sm"
                    }`}
                    style={{
                      transitionTimingFunction: "cubic-bezier(0.25, 0.46, 0.45, 0.94)",
                    }}
