...
function addSummary(doc: jsPDF, data: AssignmentExportData, y: number): number {
  ...
  y += 6;
  doc.text(`Average Team Size: ${data.metadata.averageTeamSize.toFixed(1)}`, 20, y);
  y += 6;

  if (data.metadata.averageQuality !== null) {
    doc.text(`Average Quality: ${(data.metadata.averageQuality * 100).toFixed(1)}%`, 20, y);
    y += 6;
  }
...
function addTeam(
  ...
  const headers = ["No", "Name", "NIM", "MBTI", "Gender", "EI", "SN", "TF", "PJ", "Skills"];
  ...
  team.members.forEach((member, idx) => {
    ...
    const skillsList = member.skills
      ...
      .map((s) => s.skillName)
      .join(", ");
    const skillsDisplay = member.skills.length > 3 ? `${skillsList}...` : skillsList;

    const rowData = [
      String(idx + 1),
      member.name.length > 25 ? `${member.name.substring(0, 22)}...` : member.name,
      member.nim || "-",
      member.mbtiType || "-",
...
export async function generateTeamFormationPDF(data: AssignmentExportData): Promise<Buffer> {
  const doc = new jsPDF({
