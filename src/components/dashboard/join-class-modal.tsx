  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        } else if (response.status === 403 && result.error?.includes("Demo")) {
          setError(t("demoLocked"));
        } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="onboarding" size="lg" className="rounded-full px-4 py-6 font-semibold">
      ...
      <DialogContent className="border max-w-md md:max-w-xl rounded-3xl p-0 gap-0">
        ...
        <form onSubmit={handleSubmit} className="p-6 pt-0 space-y-6">
          <div className="space-y-3">
            <label
              htmlFor="token"
              className={`text-sm font-medium block ${hasError ? "text-red-600" : "text-gray-900"}`}
            >
              {t("classCode")}
            </label>
            <motion.div
