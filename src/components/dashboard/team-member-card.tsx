...
export function TeamMemberCard({ member, topSkills, preferredTopics }: TeamMemberCardProps) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className={`text-xl font-bold ${colorScheme.primaryText}`}>{member.name}</h3>
          ...
          {member.nim && <p className={`text-sm ${colorScheme.primaryText}`}>NIM: {member.nim}</p>}
        </div>
      </div>
