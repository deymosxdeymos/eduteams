...
export function StudentManageShell({ items, isLoading }: StudentManageShellProps) {
  ...
  const notStartedCount = useMemo(
    () => items.filter((item) => item.status === "not-started").length,
    [items],
  );
            ...
            <TabsContent key={key} value={key} className="flex-1 mt-0">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  <p className="text-muted-foreground">{t("empty.noClasses")}</p>
                </div>
              ) : (
