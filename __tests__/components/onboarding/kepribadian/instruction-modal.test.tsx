...
describe('InstructionModal', () => {
  ...
  describe('Content Validation', () => {
    ...
    test('displays emphasized text correctly', () => {
      ...
      const boldTexts = screen.getAllByText((content, element) => {
        return element !== null && element.tagName === 'SPAN' && element.className.includes('font-bold');
      });
    ...
    test('displays Likert scale images with correct sources', () => {
      ...
      expectedSources.forEach(src => {
        const iconName = src.split('/').pop()?.replace('.svg', '').replace(/-/g, ' ');
        if (iconName) {
          const image = screen.getByRole('img', { name: new RegExp(iconName, 'i') });
  ...
  describe('Props Validation', () => {
    test('handles undefined onCloseAction gracefully', () => {
      ...
      render(
        <InstructionModal isOpen={true} onCloseAction={undefined as any} />
      );
