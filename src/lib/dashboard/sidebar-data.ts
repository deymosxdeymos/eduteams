...
function getCachedNotStartedCount(userId: string) {
  return unstable_cache(
    () => fetchNotStartedCount(userId),
    ['sidebar-not-started-count', userId],
    { revalidate: 30, tags: [`sidebar-data-${userId}`] }
  )();
}
...
export async function getSidebarData() {
  ...
  const notStartedCount =
    user.role === 'STUDENT' ? await getCachedNotStartedCount(user.id) : 0;
