...
describe('isValidAnswerRecord', () => {
  ...
  it('returns true for valid answer records', () => {
    ...
    expect(isValidAnswerRecord({ q1: 1, q2: 2, q3: 3, q4: 4, q5: 5 })).toBe(
      true
    );
  });
...
describe('isValidPersonalityScores', () => {
  it('returns true for valid scores', () => {
    ...
    expect(isValidPersonalityScores({ ei: -1, sn: -1, tf: -1, pj: -1 })).toBe(
      true
    );
    expect(
      isValidPersonalityScores({ ei: 0.5, sn: -0.5, tf: 0.25, pj: -0.75 })
    ).toBe(true);
  });
  ...
  it('returns false for values outside -1 to 1 range', () => {
    expect(isValidPersonalityScores({ ei: 2, sn: 0, tf: 0, pj: 0 })).toBe(
      false
    );
    expect(isValidPersonalityScores({ ei: 0, sn: -2, tf: 0, pj: 0 })).toBe(
      false
    );
    expect(isValidPersonalityScores({ ei: 1.1, sn: 0, tf: 0, pj: 0 })).toBe(
      false
    );
  });

  it('returns false for non-number values', () => {
    expect(isValidPersonalityScores({ ei: '0', sn: 0, tf: 0, pj: 0 })).toBe(
      false
    );
  });
});
...
describe('getMBTIType', () => {
  ...
  it('throws for invalid scores', () => {
    expect(() => getMBTIType({ ei: 2, sn: 0, tf: 0, pj: 0 })).toThrow(
      'Invalid personality scores'
    );
    expect(() => getMBTIType(null as any)).toThrow(
      'Invalid personality scores'
    );
  });
});
...
describe('calculatePersonalityScoresFromQuestions', () => {
  describe('single-letter dimensions', () => {
    it('handles E dimension (adds to ei)', () => {
      const questions = [
        { id: 'q1', dimension: 'e', reversed: false, isAttentionCheck: false },
      ];
    ...
    it('handles I dimension (subtracts from ei)', () => {
      const questions = [
        { id: 'q1', dimension: 'i', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
    ...
    it('handles S dimension (subtracts from sn)', () => {
      const questions = [
        { id: 'q1', dimension: 's', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
    ...
    it('handles N dimension (adds to sn)', () => {
      const questions = [
        { id: 'q1', dimension: 'n', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
    ...
    it('handles T dimension (subtracts from tf)', () => {
      const questions = [
        { id: 'q1', dimension: 't', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
    ...
    it('handles F dimension (adds to tf)', () => {
      const questions = [
        { id: 'q1', dimension: 'f', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
    ...
    it('handles J dimension (subtracts from pj)', () => {
      const questions = [
        { id: 'q1', dimension: 'j', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
    ...
    it('handles P dimension (adds to pj)', () => {
      const questions = [
        { id: 'q1', dimension: 'p', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
  ...
  describe('interaction dimensions (two-letter)', () => {
    ...
    it('interaction dimensions alone result in zero (no count)', () => {
      const questions = [
        { id: 'q1', dimension: 'nj', reversed: false, isAttentionCheck: false },
      ];
      const scores = calculatePersonalityScoresFromQuestions(
        { q1: 5 },
        questions
  ...
  describe('edge cases', () => {
    ...
    it('handles reversed questions', () => {
      const questions = [
        { id: 'q1', dimension: 'ei', reversed: true, isAttentionCheck: false },
      ];
