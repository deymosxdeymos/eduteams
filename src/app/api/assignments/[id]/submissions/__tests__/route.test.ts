...
const prismaMock: any = {
  ...
  assignment: {
    findUnique: mock(async (args: any) =>
      args?.where?.id === "a1" ? { id: "a1", courseId: "c1", structureVersion: 1 } : null,
    ),
  },
  courseEnrollment: {
    findUnique: mock(async (args: any) =>
...
describe("assignment submissions API", () => {
  it("POST creates (upserts) submission for enrolled mahasiswa", async () => {
    ...
    const res = await POST(
      new Request("http://localhost/api/assignments/a1/submissions") as any,
  ...
  it("DELETE removes submissions by course or assignment", async () => {
    ...
    const res2 = await DELETE(
      new Request("http://localhost/api/assignments/a1/submissions?assignmentId=a1") as any,
      { params: Promise.resolve({ id: "a1" }) } as any,
    );
